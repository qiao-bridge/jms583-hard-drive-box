...
传统的移动硬盘受限于 2.5 寸机械盘或 SATA 协议，容量与速度难以兼得。本项目以 **JMicron JMS583** USB 3.1 Gen 2 转 PCIe/NVMe 或 SATA 桥接芯片为核心，搭配一块 **M.2 固态硬盘**，实现一个：

- **大容量**：支持 NVMe / SATA M.2 SSD，轻松扩展到数 TB；
- **高速度**：USB 3.1 Gen 2（10Gbps）接口，实测**顺序读取约 400MB/s**；
- **低成本**：比成品硬盘盒更灵活，便于学习与二次开发。

主要面向：需要大容量高速移动存储的开发者、硬件爱好者、嵌入式数据搬运场景。

![产品整体效果](assets/渲染图.PNG)
...
- 🚀 **主控芯片**：JMicron JMS583（USB 3.1 Gen 2 to NVMe/SATA Bridge）
- 💾 **存储接口**：M.2 M-Key（NVMe） / B-Key（SATA），具体以硬件版本为准
- 🔌 **上行接口**：USB Type-C（USB 3.1 Gen 2，10Gbps）
- ⚡ **性能**：顺序读取可达 **400MB/s**（受 SSD 与线缆质量影响）
- 🧊 **散热**：PCB 预留散热片 / 导热垫位置，外壳开散热窗
- 🔧 **固件**：提供固件文件与 Windows 下测试 / 升级工具
- 📦 **外壳**：3D 打印 / CNC 外壳模型，装配即用
- ⚖ **协议支持**：UASP（USB Attached SCSI）加速，兼容 TRIM

![硬件特性一览](assets/渲染图2.png)
> JMS583 主控 + M.2 接口 + Type-C 的核心连接关系。
...
![成品效果 2](assets/_8效果2.jpg)
![成品效果 3](assets/_8效果3.jpg)
...
在 Windows / macOS 下实测，顺序读取可达约 **400MB/s**：
...
1. **焊接元器件**
   按 BOM 焊接 JMS583、晶振、LDO、电容、Type-C、M.2 插槽、风扇等。注意主控方向与小封装元件的焊接质量，焊接完成后做绿油固化。
...
2. **安装 M.2 固态**
   将 SSD 以 30° 角插入 M.2 插槽，按压后用螺丝固定。

   ![安装固态](assets/_7安装固态.jpg)
   > 将 M.2 固态插入插槽并固定。

3. **安装散热**
   在主控与 SSD 颗粒面贴导热垫、加装散热片与导流板。

   ![加装散热片](assets/_5加散热片.jpg)
   ![加装导流板](assets/_6加导流板.jpg)

4. **装入外壳 & 连接测试**
   将 PCB 装入外壳并固定，用 Type-C 数据线接入电脑，系统应自动识别为大容量存储设备。
...
1. 导出 Gerber / 直接用立创 EDA 打开工程；
2. 按默认工艺下单（板厚 1.6mm，沉金或喷锡均可）；
3. 收到板子后按 BOM 焊接。

### 打印 / 制作外壳

见 [`enclosure/`](./enclosure) 目录，包含 3D 打印模型（STL/STEP）或 CNC 图纸。

- 3D 打印建议：PLA / PETG，层高 0.2mm；
- 装配时先放 PCB，再扣合上下壳，用螺丝或卡扣固定。装配分解见 [实物展示](#实物展示) 中的分步图与 [`分解效果.mp4`](assets/分解效果.mp4)。
- 外壳模型渲染见 [项目简介](#项目简介)（[`渲染图.PNG`](assets/渲染图.PNG)），成品效果见 [实物展示](#实物展示)（[`_8效果1.jpg`](assets/_8效果1.jpg) 等）。

### 焊接元器件

- 推荐使用**热风枪 + 锡膏**焊接 JMS583（QFN 封装）等小元件；
...
**Q：速度达不到 400MB/s？**
A：请依次检查：① 使用 **USB 3.1 Gen 2（10Gbps）数据线**；② 插入主板 **蓝色 USB 3.0+ 接口**；③ SSD 本身性能（老款 SATA M.2 上限约 500MB/s，NVMe 才能更高）；④ 是否启用 UASP。

**Q：电脑无法识别硬盘盒？**
A：① 换线、换接口排除线缆问题；② 确认 SSD 已正确插入并固定；③ 检查磁盘管理器是否需初始化；④ 查看设备管理器是否有未知设备（可能需要驱动）。

**Q：发热明显正常吗？**
A：NVMe SSD 与桥接芯片在高负载下会发热，属正常现象。建议贴导热垫 + 加装外壳散热窗；持续拷贝大文件时可间歇停顿。

**Q：支持哪些操作系统？**
A：USB Mass Storage 为标准协议，Windows / macOS / Linux 均即插即用，无需额外驱动。
